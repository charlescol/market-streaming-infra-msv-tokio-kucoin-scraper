...
pub async fn get_public_token(host: &str) -> Result<(String, String)> {
    ...
    let endpoint = resp.data.instance_servers.first().ok_or(anyhow::anyhow!("No instance servers found"))?.endpoint.clone();
...
pub async fn connect_kucoin(
    ...
    let url = format!("{}?token={}&connectId={}", endpoint, token, uuid::Uuid::new_v4());
    let uri: http::Uri = url.parse().map_err(|e: http::uri::InvalidUri| WebSocketError::CannotConnect(e.to_string()))?;
    
    let host = uri.host().ok_or(WebSocketError::CannotConnect("No host in URI".into()))?;
    ...
    let tls_connector = TlsConnector::new().map_err(|e| WebSocketError::CannotConnect(e.to_string()))?;
